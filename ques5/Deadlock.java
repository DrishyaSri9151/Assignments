public class Deadlock {
    public static void main(String[] args) throws InterruptedException {
        ...
        Account account1 = new Account("186400", 10000);
        Account account2 = new Account("3247", 20000);
        Thread t1 = new Thread(() -> {
            for (int i = 0; i < 500; i++) {
                deadlock.transfer(account1, account2, 10);
            }
        });
        Thread t2 = new Thread(() -> {
            for (int i = 0; i < 500; i++) {
                deadlock.transfer(account2, account1, 10);
            }
        });
        t1.start();
        t2.start();
